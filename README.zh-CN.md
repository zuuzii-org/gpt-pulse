<p align="center">
  <img src="Assets/Brand/GPTPulse-AppIcon-Rendered-512.png" width="128" height="128" alt="GPT Pulse 应用图标">
</p>

# GPT Pulse — Codex Desktop macOS 任务监控工具

<p align="center"><strong>让 Codex 任务状态始终可见。</strong></p>
...
<p align="center">
  <a href="https://github.com/zuuzii-org/gpt-pulse/releases/latest">下载</a> ·
  <a href="docs/ARCHITECTURE.md">架构</a> ·
  <a href="LICENSE">MIT License</a>
</p>

**GPT Pulse 是一款面向 Codex Desktop 的开源原生 macOS 菜单栏任务监控工具。** 它将正在运行、等待授权或回答、最近完成的任务，以及活跃 Agent 总数、token 用量和额度重置时间集中到一个右侧边栏中。GPT Pulse 只读取本地 Codex 数据，不会修改 Codex 任务记录。

应用界面目前使用简体中文，项目文档提供英文和简体中文版本。

<p align="center">
  <img src="Assets/Release/product-sidebar.png" width="400" alt="GPT Pulse 侧边栏，显示 Codex 运行任务、授权请求、活跃 Agent、最近完成、token 用量和额度重置时间">
</p>
...
[下载最新的已签名、已公证 DMG](https://github.com/zuuzii-org/gpt-pulse/releases/latest)。GPT Pulse v1.2.0 要求 macOS 14 或更高版本，同时支持 Apple Silicon 和 Intel Mac。
...
- **可折叠任务分组。** “正在运行”和“最近完成”可以分别折叠；GPT Pulse 会在下次启动时恢复你的选择。
- **需要注意的状态。** 等待授权和等待回答的任务会优先呈现；失败和中断状态会在最近列表中明确标识。
- **有用的任务上下文。** 每项显示项目、session、持续时间、最后状态、累计 token，以及主 Agent 和全部层级子 Agent 的活跃总数。
- **直接定位任务。** 点击任务会通过 `codex://threads/<thread-id>` 打开对应 Codex Desktop 任务。从 GPT Pulse 打开完成任务后会自动标记为已查看，也支持手动和批量确认。
...
- **符合 macOS 使用习惯。** 支持开机启动、多显示器、减少动态效果，以及配置是否在全屏应用中启用触边。

最近完成任务保留 24 小时，最多显示 20 条；未查看的成功任务优先进入保留集合。批量标记已查看后，可以在六秒内撤销。

## 工作原理

GPT Pulse 使用几层职责明确的本地适配器，不把任何一种 Codex 私有格式当作永久契约：
...
4. GPT Pulse 通过本机连接查询 Codex bundled App Server 的 `account/rateLimits/read`，获得与 Codex Desktop 同组的 5 小时和每周额度。
5. 已查看回执与 GPT Pulse 偏好独立保存在 `~/Library/Application Support/GPT Pulse/`。
...
1. 从 [GitHub Releases](https://github.com/zuuzii-org/gpt-pulse/releases/latest) 下载 DMG 和对应的 SHA-256 文件。
2. 使用上面的命令验证校验和。
3. 打开 DMG，将 `GPT Pulse.app` 拖入 `Applications`。
4. 从 `Applications` 启动 GPT Pulse。通知权限是可选的，不授权也能在本机监控任务。
5. 如果需要，可在 GPT Pulse 设置中开启开机启动。

## 应用内更新

GPT Pulse v1.1.0 开始支持应用内更新。右键点击菜单栏项目，选择“检查更新…”，即可检查公开的 GitHub Release feed 并安装新版本。

v1.1.0 是更新通道的 bootstrap 版本。v1.0.0 用户必须先手动下载并安装一次 v1.1.0；之后的版本可以直接在 GPT Pulse 内安装。
...
GPT Pulse 是一款监控本机 Codex Desktop 任务的原生 macOS 菜单栏应用。它将任务状态和用量汇总到右侧边栏，让你不必一直打开每个任务窗口。

### 如何在 macOS 上同时监控多个 Codex Desktop 任务？

让 GPT Pulse 与 Codex Desktop 同时运行即可。菜单栏会汇总正在执行和最近完成的数量，右侧边栏则逐项显示每个本机根任务的状态、项目、持续时间、token 用量和活跃 Agent 总数。

### GPT Pulse 会修改或控制 Codex 任务吗？

不会。它读取 Codex 数据，并可以通过 deep link 打开已有任务，但不会批准、回答、停止、重试、创建、归档或编辑任务。

### GPT Pulse 会上传 prompt 或任务数据吗？

不会。任务监控留在本机。GPT Pulse 不会提取、保留或上传 prompt、tool input、tool output 或 transcript，也不需要 OpenAI API Key。

### 不安装 Codex 插件可以使用吗？

可以。插件完全可选。不安装时，GPT Pulse 使用只读 SQLite 和 rollout JSONL；部分运行中或等待授权的状态变化可能出现得稍晚。

### 会显示子 Agent 吗？

子 Agent 不会成为独立任务行。GPT Pulse 将主 Agent 与全部活跃后代聚合为根任务的 `Agent N`。如果本地证据不完整，会显示未知或可能过期，而不是把未知伪装成 `0`。

### 5 小时与每周数字代表什么？

它们是根据 Codex 提供的已用百分比换算出的剩余百分比，同时附带重置时间，并不是绝对 token 余额。GPT Pulse 只接受同一完整快照中的两个窗口，不会跨任务拼接数值。

### GPT Pulse 如何读取 Codex 使用额度？

它通过本机 Codex App Server 读取与 Codex Desktop 设置页一致的 5 小时和每周整组额度。兼容的本地 rollout 数据仅作为兜底，并且不会拼接来自不同快照的两个窗口。

### 能否从 GPT Pulse 直接打开 Codex Desktop 任务？

可以。点击任务行会通过本机 `codex://threads/<thread-id>` 链接打开对应任务。如果跳转失败，GPT Pulse 会保留未查看状态并显示错误，不会静默确认任务。
...
不支持。当前范围仅包括本机 Codex Desktop 创建的根任务。

### GPT Pulse 是 OpenAI 官方产品吗？

不是。GPT Pulse 是 Zuuzii 独立开发的开源项目，与 OpenAI 无隶属关系，也未获得 OpenAI 的背书或维护。
...
```bash
git clone https://github.com/zuuzii-org/gpt-pulse.git
cd gpt-pulse
make check
...
GPT Pulse 使用 [MIT License](LICENSE) 开源，产品署名为 **Zuuzii**。

GPT Pulse 是独立项目，与 OpenAI 无隶属关系，也未获得 OpenAI 的背书或维护。文中产品和公司名称仅用于说明兼容范围。
