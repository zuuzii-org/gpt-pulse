...
struct CodexPaths: Sendable {
    ...
    let compatibilityPluginJournalURLs: [URL]
    ...
    static func live(
        ...
        let applicationSupportResolution = LegacyCompatibility
            .resolveApplicationSupportDirectory(homeDirectory: homeDirectory)
        let applicationSupport = applicationSupportResolution.directoryURL
        let currentApplicationSupport = LegacyCompatibility
            .currentApplicationSupportURL(homeDirectory: homeDirectory)
            .standardizedFileURL
        let legacyApplicationSupport = LegacyCompatibility
            .legacyApplicationSupportURL(homeDirectory: homeDirectory)
            .standardizedFileURL
        var journalRootCandidates = [
            applicationSupport.standardizedFileURL,
            currentApplicationSupport,
        ]
        if applicationSupportResolution.state != .unsafeLegacySource
            && applicationSupportResolution.state != .unsafeCurrentDestination {
            journalRootCandidates.append(legacyApplicationSupport)
        }
        let journalRoots = journalRootCandidates.reduce(into: [URL]()) { result, candidate in
            guard !result.contains(where: { $0.path == candidate.path }) else { return }
            result.append(candidate)
        }
        let journalURLs = journalRoots.map {
            $0.appendingPathComponent("events", isDirectory: true)
        ...
        return CodexPaths(
            ...
            pluginJournalURL: journalURLs[0],
            compatibilityPluginJournalURLs: Array(journalURLs.dropFirst()),
