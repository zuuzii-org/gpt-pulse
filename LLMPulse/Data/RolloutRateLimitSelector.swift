...
/// when more than one weekly reset generation is still plausible. Legacy 5h
/// data is retained on the selected snapshot but does not affect eligibility.
enum RolloutRateLimitSelector {
    ...
    static func select(
        ...
        for snapshot in pool.sorted(by: { $0.updatedAt < $1.updatedAt }) {
            if let index = groups.firstIndex(where: { group in
                ...
                return hasSameWeeklyReset(snapshot, representative)
            }) {
    ...
    private static func isValid(_ snapshot: RateLimitSnapshot, now: Date) -> Bool {
        guard snapshot.conflictingResetHistoryUntil.map({ $0 <= now }) ?? true,
              ...
              isValid(
                  weekly,
                  expectedMinutes: RateLimitWindowDuration.weeklyMinutes,
                  snapshot: snapshot,
                  now: now
              )
        else {
    ...
    private static func oldestObservation(in snapshot: RateLimitSnapshot) -> Date {
        snapshot.weekly?.observedAt ?? snapshot.updatedAt
    }

    private static func hasSameWeeklyReset(
        _ lhs: RateLimitSnapshot,
        _ rhs: RateLimitSnapshot
    ) -> Bool {
        guard let lhsWeekly = lhs.weekly,
