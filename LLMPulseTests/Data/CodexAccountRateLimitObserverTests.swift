...
final class CodexAccountRateLimitObserverTests: XCTestCase {
    ...
    func testRefreshWithoutWeeklyWindowIsRejectedExplicitly() async {
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        let loader = ControlledRateLimitLoader()
        let observer = CodexAccountRateLimitObserver(
            loader: loader,
        ...
        await loader.succeedNext(with: RateLimitSnapshot(
        ...
        let observation = await observer.observation(now: now.addingTimeInterval(1))

        XCTAssertNil(observation.snapshot)
        XCTAssertEqual(observation.health.status, .unavailable)
        XCTAssertEqual(
            observation.health.message,
            CodexAppServerRateLimitError.malformedResponse.localizedDescription
        )
        XCTAssertTrue(observation.fallbackAllowed)
    }
