...
final class CodexAppServerRateLimitClientTests: XCTestCase {
    ...
    func testRealWeeklyOnlyResponseShapeAcceptsMissingSecondary() throws {
        let fixtureURL = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .appendingPathComponent("Fixtures", isDirectory: true)
            .appendingPathComponent("codex-account-rate-limits-weekly-only.json")
        ...
        XCTAssertEqual(snapshot.weekly?.usedPercent, 2)
        XCTAssertEqual(snapshot.weekly?.windowMinutes, 10_080)
        XCTAssertEqual(snapshot.weekly?.resetsAt, Date(timeIntervalSince1970: 1_800_600_000))
        XCTAssertEqual(snapshot.weekly?.observedAt, observedAt)
    }

    func testMalformedLegacyFiveHourWindowDoesNotInvalidateWeeklyWindow() throws {
        let observedAt = Date(timeIntervalSince1970: 1_700_000_000)
        let weeklyReset = observedAt.addingTimeInterval(7 * 24 * 60 * 60)
        let response: [String: Any] = [
            "result": [
                "rateLimits": [
                    "limitId": "codex",
                    "primary": window(used: 4, minutes: 10_080, reset: weeklyReset),
                    "secondary": [
                        "usedPercent": 99,
        ...
        XCTAssertNil(snapshot.fiveHour)
        XCTAssertEqual(snapshot.weekly?.usedPercent, 4)
    }

    func testNonCanonicalTopLevelUsesCanonicalMapFallback() throws {
        let observedAt = Date(timeIntervalSince1970: 1_700_000_000)
        let response: [String: Any] = [
            "result": [
                "rateLimits": [
                    ...
                    "primary": window(
                        used: 91,
                        minutes: 10_080,
                        reset: observedAt.addingTimeInterval(600)
                    ),
                ],
                "rateLimitsByLimitId": [
                    "codex": [
    ...
    func testTopLevelWithoutLimitIDUsesCanonicalMapFallback() throws {
        let observedAt = Date(timeIntervalSince1970: 1_700_000_000)
        let response: [String: Any] = [
            "result": [
                "rateLimits": [
    ...
    func testMalformedCanonicalTopLevelDoesNotSilentlyUseMapFallback() {
        let observedAt = Date(timeIntervalSince1970: 1_700_000_000)
        let response: [String: Any] = [
            "result": [
                "rateLimits": [
                    ...
                    "primary": window(
                        used: 7,
                        minutes: 300,
                        reset: observedAt.addingTimeInterval(300)
                    ),
                ],
                "rateLimitsByLimitId": [
                    "codex": [
    ...
    func testUnknownWindowDurationWithoutWeeklyIsRejected() {
        let observedAt = Date(timeIntervalSince1970: 1_700_000_000)
        let response: [String: Any] = [
            "result": [
                "rateLimits": [
                    ...
                    "primary": window(
                        used: 14,
                        minutes: 1_440,
                        reset: observedAt.addingTimeInterval(24 * 60 * 60)
                    ),
                ],
            ],
        ]
