...
final class TaskRepositoryTests: XCTestCase {
    func testUpgradeOnlyPluginJournalRemainsReadableWithoutWritingLegacyState() async throws {
        ...
        try writeRunningRollout(
            to: sessions.appendingPathComponent("compatibility.jsonl"),
            threadId: "compatibility-thread",
            turnId: "compatibility-turn",
            startedAt: now.addingTimeInterval(-60)
        )
        let compatibilityJournal = root.appendingPathComponent("upgrade-events.jsonl")
        try writeJSONLines([
            [
                "session_id": "compatibility-thread",
                "turn_id": "compatibility-turn",
                "cwd": "/tmp/project",
                "hook_event_name": "PermissionRequest",
                "timestamp": now.addingTimeInterval(-5).ISO8601Format(),
            ],
        ], to: compatibilityJournal)
        ...
        let repository = TaskRepository(
            ...
            journalReader: PluginEventJournalReader(
                journalURL: root.appendingPathComponent("current-events-missing.jsonl")
            ),
            additionalJournalReaders: [
                PluginEventJournalReader(journalURL: compatibilityJournal),
            ],
        ...
        XCTAssertEqual(snapshot.tasks.first?.state, .waitingForApproval)
        XCTAssertEqual(
            snapshot.health.first { $0.adapter == .pluginJournal }?.status,
            .healthy
        )
        XCTAssertFalse(
            FileManager.default.fileExists(
                atPath: root.appendingPathComponent("current-events-missing.jsonl").path
            )
        )
    }
