...
final class NotificationServiceTests: XCTestCase {
    ...
    func testQuotaAlertsTrackFiveHourAndWeeklyWindowsIndependently() {
        let suiteName = "NotificationServiceTests.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defer { defaults.removePersistentDomain(forName: suiteName) }
        ...
        XCTAssertEqual(Set(alerts.map(\.windowMinutes)), [300, 10_080])
        XCTAssertEqual(Set(alerts.map(\.threshold)), [20, 10])
    }

    func testQuotaFreshnessIsEvaluatedPerWindow() {
        let suiteName = "NotificationServiceTests.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defer { defaults.removePersistentDomain(forName: suiteName) }
        ...
        XCTAssertEqual(alerts.map(\.windowMinutes), [300])
        XCTAssertEqual(alerts.map(\.threshold), [20])
    }
