...
enum CodexAppServerRateLimitResponseParser {
    static func parse(
        ...
        let canonicalRateLimits = (result["rateLimitsByLimitId"] as? [String: Any])?["codex"]
            as? [String: Any]
        guard let rateLimits = topLevelRateLimits ?? canonicalRateLimits else {
            throw CodexAppServerRateLimitError.malformedResponse
        }
        ...
        return RateLimitSnapshot(
            ...
            limitID: JSONValueSupport.string(rateLimits["limitId"])
        )
    }
...
private final class CodexAppServerRateLimitWorker: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.zuuzii.GPTPulse.app-server-rate-limits")
